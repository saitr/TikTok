...
async function getVideosAndHashTags() {
  const videoElements = await page.$x('//*[@id="main-content-others_homepage"]/div/div[2]/div[2]/div//a');
  const videos = videoElements.map(video => video.href).filter(href => href !== null);
  
  const hashtagElements = await page.$x('//*[@id="main-content-others_homepage"]/div/div[2]/div[2]/div//a');
  const hashtags = hashtagElements.map(hashtag => hashtag.getAttribute('title')).filter(title => title !== null);

  const result = {};

  for (let i = 0; i < hashtags.length; i++) {
    result[hashtags[i]] = videos[i] || '';
  }

  return result;
}

const result = await getVideosAndHashTags();
console.log('**************************************videos and hashtags****************************************', result);
